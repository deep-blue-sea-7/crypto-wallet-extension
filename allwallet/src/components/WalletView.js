      ...
      children: (
        <>
          {tokens ? (
              ...
              <div className="tokens">
                <List
                  ...
                  renderItem={(item, index) => (
                    <List.Item className="tokens" style={{ textAlign: "left" }}>
                      <List.Item.Meta className="tokens" avatar={<Avatar src={item.logo || logo} />} title={item.symbol} description={item.name} />
            ...
            <>
              <span>You do have any tokens yet</span>
            </>
          )}
        </>
      ),
      ...
      children: (
        <>
          {nfts ? (
            <>
              {nfts.map((e, i) => {
                return <>{e && <img key={i} className="nftImage" alt="nftImage" src={e} />}</>;
              })}
            </>
          ) : (
            <>
    ...
    },
    {
      ...
      children: (
        <>
          ...
          <Button className="frontPageButton" onClick={() => sendTransaction(sendToAddress, amountToSend)}>
            Send Tokens
          </Button>
          {processing && (
            <>
  ...
  return (
    <>
      <div className="content">
        ...
        <Divider />
        {fetching ? <Spin /> : <Tabs defaultActiveKey="3" items={items} className="walletView" />}
      </div>
    </>
  );
}
